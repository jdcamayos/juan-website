...
export default function ProjectsContainer({ projects, title }) {
	const [projectSelected, setProjectSelected] = useState(projects[0])

	const setScale = (url, width, height) =>
		String(url).replace(
			'upload/',
			`upload/c_scale${width ? `,w_${width ? width : 500}` : ''}${height ? `,h_${height ? height : 500}` : ''}/`
		)
	...
	return (
		<>
			...
			<main className='container projects__container'>
				<Link href="/#work">
					<span className='projects__goback-button'>
						<FontAwesomeIcon icon={faArrowLeft} />
					</span>
				</Link>
				<h1 className='section__title'>{title} projects</h1>
				<section className='project__section'>
						...
						<div className='project__carousel-viewport'>
							{projectSelected.img.map((img, i) => (
								<article key={i} className='project__carousel-slide'>
									<img src={setScale(img, 800)} alt={projectSelected.title} />
								</article>
							))}
						</div>
					</div>
					<div>
						...
						<div className='project__complexity'>
							{Array.from({ length: projectSelected.complexity }).map((_, i) => (
								<span key={i}>
					...
					<div className='project__list-viewport'>
						{projects.map(p => (
							<article
								...
								className={`projects__list-item ${p.id === projectSelected.id ? 'project__item-selected' : ''}`}
							>
								<img src={setScale(p.img[0], null, 100)} alt={p.title} className='project__item-img' />
